...
The discovery call is `{"action":"interact","payload":{}}`. Submit classifications inside `payload.proposal.operations`, never `payload.operations`. For example, use `{"action":"interact","payload":{"proposal":{"operations":[{"intent":"GENERAL_WORK","action":"write","start":0,"end":RETURNED_END}]}}}` with the actual returned number. Do not invent subject_id for a new job. Keep payload as a JSON object, not a JSON string. Choose `read` for analysis, `write` for ordinary file edits and `run` only for engineering execution.
