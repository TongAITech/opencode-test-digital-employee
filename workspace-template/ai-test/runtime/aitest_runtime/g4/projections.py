...
from .contracts import EXTENSION_ID, G4Fact, G4State

TABLE = "g4_real_execution_facts"
SQL = """
CREATE TABLE g4_real_execution_facts (
...
class G4ProjectionContribution:
    ...
    def apply(self, conn: sqlite3.Connection, composed: ComposedRuntimeState) -> None:
        ...
        if not isinstance(state, G4State):
            raise RuntimeError("G4_PROJECTION_INPUT_INVALID", EXTENSION_ID)
    ...
    def read(self, conn: sqlite3.Connection, mission_id: str) -> G4State:
        ...
        facts = [G4Fact.from_dict(json.loads(row[0])) for row in rows]
        facts.sort(key=lambda fact: fact.created_seq)
        return G4State(mission_id, tuple(facts))
    ...
    def verify(self, replayed_state: G4State, projected_state: G4State | None) -> dict[str, object]:
