...
def _control_intent(row, value):
    _require(row.get('intent')=='MISSION_CONTROL' and row.get('action') in {'pause','stop','continue'}, 'INTERACTION_CONTROL_INTENT_INVALID')
    _require(isinstance(value, Mapping) and set(value)=={'mission_id','admission_seq','control_generation','from_status','target_status','no_effect'}, 'INTERACTION_CONTROL_INTENT_INVALID')
    _require(row.get('subject')=={'subject_kind':'MISSION','subject_id':value['mission_id']}, 'INTERACTION_CONTROL_TARGET_INVALID')
    _require(type(value['admission_seq']) is int and type(value['control_generation']) is int
        and 0 < value['control_generation'] <= value['admission_seq'], 'INTERACTION_CONTROL_CURSOR_INVALID')
    _require(value['target_status']=={'pause':'PAUSED','stop':'CANCELLED','continue':'ACTIVE'}[row['action']]
        and value['from_status'] in {'CREATED','ACTIVE','PAUSED','BLOCKED','COMPLETED','FAILED','CANCELLED'}
        and type(value['no_effect']) is bool and value['no_effect']==(value['from_status']==value['target_status']), 'INTERACTION_CONTROL_DECISION_INVALID')
    return dict(value)
...
class CommandContribution:
    def handle(self, command, composed):
        ...
        if command.type == 'CLAIM_INTERACTION_OPERATION' and 'control_intent' in command.payload['operation']:
            ...
            return [PendingEvent(COMMAND_EVENT[command.type], ROOT_KIND, command.mission_id, base),
                    PendingEvent(CONTROL_INTENT_EVENT, ROOT_KIND, command.mission_id,
                        {'subject':base['subject'],'control_intent':intent,'request_digest':operation['request_digest']})]
    ...
    def reduce(self, state, event, core_state):
        ...
        if event.event_type == CONTROL_INTENT_EVENT:
            ...
            _require(set(data)=={'subject','control_intent','request_digest'} and data['subject']=={'subject_kind':ROOT_KIND,'subject_id':state.subject_id}
                and row.get('state')=='CLAIMED' and 'control_intent' not in row and data['request_digest']==row['request_digest'], 'INTERACTION_CONTROL_INTENT_INVALID')
            return replace(state,receipt={**row,'control_intent':_control_intent(row,data['control_intent'])})
