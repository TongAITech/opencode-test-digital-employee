...
def main():
    ...
    if not path: return emit({"ok":False,"status":"NOT_CONFIGURED","required":"file_path or bind an approved Starlink adapter"})
    p=Path(path)
    if not p.exists(): return emit({"ok":False,"status":"SOURCE_NOT_FOUND","path":str(p)})
    emit({"ok":True,"status":"CURRENT_RELEASE_TRUTH","payload":json.loads(p.read_text(encoding="utf-8")),"source_ref":str(p)})
