...
class BrowserPort:
    def __init__(self, ref: BrowserContextRef): self.identity=ref; self.owner='AI'; self.handoffs=0
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='g4-full-e2e-') as td:
  ...
  try:
   ...
   goal=product_entry.g4_command('DIRECTOR','create_goal',{'mission_id':mid,'goal_id':'goal-95','project_id':'PFC','release_id':'V2','requirement_scope':['REQ-018'],'affected_applications':['cfg-data','cfg-admin'],'coverage_policy':{'target_pct':95,'aggregation_policy':'PER_AFFECTED_APPLICATION','critical_gap_policy':'ZERO_UNRESOLVED_CRITICAL'}}); checks['g4_goal_95_durable']=goal['goal']['payload']['coverage_policy']['target_pct']==95.0
   ...
   case_payload=initial_cases[0]['payload']['r3_3_case']; case_id=str(case_payload['tc_id']); case_ver=str(case_payload['case_version_id'])
   g4.record_cursor(mid,{'task_id':ab['task_id'],'attempt_id':ab['attempt_id'],'case_id':case_id,'case_version':case_ver,'current_step_index':0,'completed_step_ids':[],'pending_step_id':'login','last_safe_checkpoint':'before-login'})
   ...
   g4r.record_cursor(mid,{'task_id':arb['task_id'],'attempt_id':arb['attempt_id'],'case_id':case_id,'case_version':case_ver,'current_step_index':1,'completed_step_ids':['login'],'pending_step_id':'confirm','last_safe_checkpoint':'after-login'})
   ...
   auto=product_entry.g4_command('EXECUTOR','complete_human_takeover',{'mission_id':mid,'human_gate_id':'g4-auto','completion_mode':'AUTO','resume_condition_verified':True,'verification':{'auth_state':'VERIFIED','page_identity':'VERIFIED','business_state':'RESUME_SAFE'},'source_ref':'supervisor:auto','actor_id':'g4-browser-supervisor'}); checks['deterministic_auto_resume']=auto['cursor']['payload']['current_step_index']==1 and bp.owner=='AI'
   ...
   g4r.record_cursor(mid,{'task_id':cb['task_id'],'attempt_id':cb['attempt_id'],'case_id':str(ncp['tc_id']),'case_version':str(ncp['case_version_id']),'current_step_index':0,'completed_step_ids':[],'pending_step_id':'execute','last_safe_checkpoint':'ready'})
   stage('execute governed batch2: capability')
   product_entry.g4_command('EXECUTOR','execute_capability',{**cb,'capability_id':'API','case_id':str(ncp['tc_id']),'case_version':str(ncp['case_version_id']),'executor_request':{'url':'https://sut.test/limits','method':'POST','authorized_scope':{'environment':'TEST'}},'step':{'step_id':'execute','expected':'critical line reached + invariant pass','fixture_actual':'critical line reached + invariant pass'},'execution_node':'node-c'})
