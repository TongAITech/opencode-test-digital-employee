...
def g3_cycle(mid, orch, coverage_box, repos, cycle, replan_ref=None):
 ...
 if cycle > 1:
  ...
  if planner_session.get('status') not in {'PLANNER_SESSION_OPEN','OPENED','ALREADY_OPEN','PASS'}:
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='g4-full-e2e-') as td:
  ...
  try:
   ...
   plan1=orch.propose_plan(mid,{'objective':'execute G3 governed batch 1','tasks':[exec_task('EXEC-A',case_refs[0]),exec_task('EXEC-B',case_refs[-1])],'dependencies':[]}); a=plan1['next']; ab=binding(a); g4.create_batch(mid,{'batch_id':'batch-1','goal_id':'goal-95','case_refs':case_refs,'strategy_version_id':strategy1,'target_application':'cfg-data','status':'RUNNING'})
   ...
   invoke_model_command(orch_restart,family='g4',role='EXECUTOR',action='request_human_takeover',payload={**arb,'human_gate_id':'g4-auto','takeover_id':'tk-auto','case_id':case_id,'browser_context_ref':bp._ref().to_dict(),'required_action':'confirm protected page state','reason':'AUTH_REVERIFY','allowed_scope':{'environment':'TEST'},'resume_mode':'AUTO','resume_condition':{'protected_probe':'pass'},'goal_id':'goal-95','mandatory_for_goal':True})
   not_ready=TestObjectiveController(g4r).tick(mid,'goal-95',replan_context={}); auto_waited=bp.owner=='HUMAN' and not_ready['auto_resume']['status']=='WAITING'
   bp.resume_ready=True
   auto_tick=TestObjectiveController(g4r).tick(mid,'goal-95',replan_context={}); auto=G4RealExecutionService(runtime_restart,orchestration=orch_restart,browser_provider=bp).recover_cursor(mid,root_attempt_id=arb['root_attempt_id']); checks['deterministic_auto_resume']=auto_waited and auto['payload']['current_step_index']==1 and bp.owner=='AI' and 'g4-auto' in auto_tick['auto_resume']['resumed_gate_refs']
   ...
   invoke_model_command(orch_restart,family='g4',role='EXECUTOR',action='execute_capability',payload={**cb,'capability_id':'API','case_id':str(ncp['tc_id']),'case_version':str(ncp['case_version_id']),'case_spec_fact_id':newcase['fact_id'],'execution_batch_id':'batch-2','executor_request':{'url':'https://sut.test/limits','method':'POST','authorized_scope':{'environment':'TEST'}},'step':{'step_id':'execute','expected':'critical line reached + invariant pass','fixture_actual':'critical line reached + invariant pass'},'execution_node':'node-c'})
