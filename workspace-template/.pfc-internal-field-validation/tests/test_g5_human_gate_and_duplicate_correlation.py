...
def worker(orch,action,payload):
 original=product_entry.orchestration_service
 product_entry.orchestration_service=lambda _root=None: orch
 try: return invoke_g5_worker(orch,action,payload)
 finally: product_entry.orchestration_service=original
...
def main():
 ...
 if callable(command) and hunter is not None:
  with tempfile.TemporaryDirectory(prefix='g5-human-') as td:
   ...
   try:
    rt,orch,mid,b,first=mk(root,'main'); s=seed(rt,mid,'main'); before=count(rt,mid); v,e=invoke(lambda:worker(orch,'assess_defect_truth',ap(b,s,HIGH))); behavior['no_gate_blocks_confirmation']=safe_non_confirmed(v,e) and count(rt,mid)==before
    gid='g5-human-main'; _,gate_exc=invoke(lambda:command('DIRECTOR','request_human_review',{**b,'candidate_id':s['candidate_id'],'gate_id':gid,'policy_context':HIGH})); r26=HumanGateApplicationService(rt); g=(r26.state(mid).gate(gid) or next((x for x in r26.state(mid).gates if x.task_id==b['task_id']),None)) if gate_exc is None else None
    if g:
  ...
  for mode in ('reject','more'):
   with tempfile.TemporaryDirectory(prefix='g5-human-'+mode+'-') as td:
    root=Path(td); old=(os.environ.get('AITEST_WORKSPACE_ROOT'),os.environ.get('AITEST_RUNTIME_SPINE_DB')); os.environ['AITEST_WORKSPACE_ROOT']=str(root); os.environ['AITEST_RUNTIME_SPINE_DB']=str(root/'runtime-spine.db')
    try:
     rt,orch,mid,b,_=mk(root,mode); s=seed(rt,mid,mode); gid='g-'+mode; _,gate_exc=invoke(lambda:command('DIRECTOR','request_human_review',{**b,'candidate_id':s['candidate_id'],'gate_id':gid,'policy_context':HIGH})); r=HumanGateApplicationService(rt); g=(r.state(mid).gate(gid) or next((x for x in r.state(mid).gates if x.task_id==b['task_id']),None)) if gate_exc is None else None
     if g:
      ...
      v,e=invoke(lambda:worker(orch,'assess_defect_truth',ap(b,s,HIGH))); behavior['rejected_block_blocks' if mode=='reject' else 'plan_revision_more_evidence_blocks']=safe_non_confirmed(v,e) and count(rt,mid)==0
    finally:
     if old[0] is None: os.environ.pop('AITEST_WORKSPACE_ROOT',None)
     else: os.environ['AITEST_WORKSPACE_ROOT']=old[0]
     if old[1] is None: os.environ.pop('AITEST_RUNTIME_SPINE_DB',None)
  ...
  with tempfile.TemporaryDirectory(prefix='g5-human-insufficient-') as td:
   root=Path(td); old=(os.environ.get('AITEST_WORKSPACE_ROOT'),os.environ.get('AITEST_RUNTIME_SPINE_DB')); os.environ['AITEST_WORKSPACE_ROOT']=str(root); os.environ['AITEST_RUNTIME_SPINE_DB']=str(root/'runtime-spine.db')
   try:
    rt,orch,mid,b,_=mk(root,'ins'); s=seed(rt,mid,'ins',False); gid='g-ins'; _,gate_exc=invoke(lambda:command('DIRECTOR','request_human_review',{**b,'candidate_id':s['candidate_id'],'gate_id':gid,'policy_context':HIGH})); r=HumanGateApplicationService(rt); g=(r.state(mid).gate(gid) or next((x for x in r.state(mid).gates if x.task_id==b['task_id']),None)) if gate_exc is None else None
    if g:
     r.record_decision({'mission_id':mid,'gate_id':g.gate_id,'decision_id':'c','outcome':'CHOICE_SELECTED','route':'RESUME_EXECUTION','decision_payload':{'choice':'CONFIRM_DEFECT'},'actor':{'type':'USER','id':'u'}}); olda=b['attempt_id']; orch.rotate_session(mid,task_id=b['task_id'],reasons=['CONTROL_OVERRIDE']); latest=rt.replay_composed(mid).extension_state('r1_3b_execution_resume').latest_attempt(b['task_id']); nb={**b,'attempt_id':latest.attempt_id,'session_id':latest.runtime_session_id}; r.record_continuation({'mission_id':mid,'gate_id':g.gate_id,'route':'RESUME_EXECUTION','canonical_reference':{'successor_attempt_id':latest.attempt_id,'successor_session_id':latest.runtime_session_id,'predecessor_attempt_id':olda,'successor_root_attempt_id':latest.root_attempt_id},'continuation_operation_id':'a','actor':{'type':'SYSTEM','id':'ec0'}}); v,e=invoke(lambda:worker(orch,'assess_defect_truth',ap(nb,s,HIGH))); behavior['human_cannot_bypass_r36']=safe_non_confirmed(v,e) and count(rt,mid)==0
   finally:
    if old[0] is None: os.environ.pop('AITEST_WORKSPACE_ROOT',None)
    else: os.environ['AITEST_WORKSPACE_ROOT']=old[0]
    if old[1] is None: os.environ.pop('AITEST_RUNTIME_SPINE_DB',None)
  ...
  with tempfile.TemporaryDirectory(prefix='g5-ordinary-') as td:
   root=Path(td); old=(os.environ.get('AITEST_WORKSPACE_ROOT'),os.environ.get('AITEST_RUNTIME_SPINE_DB')); os.environ['AITEST_WORKSPACE_ROOT']=str(root); os.environ['AITEST_RUNTIME_SPINE_DB']=str(root/'runtime-spine.db')
   try:
    rt,orch,mid,b,_=mk(root,'ordinary'); s=seed(rt,mid,'ordinary'); r=HumanGateApplicationService(rt); n=len(r.state(mid).gates); _,assessment_exc=invoke(lambda:worker(orch,'assess_defect_truth',ap(b,s,ORDINARY))); a=R36ApplicationService(rt).state(mid).defect_assessment(s['assessment_id']) if assessment_exc is None else None; behavior['ordinary_needs_no_gate']=assessment_exc is None and len(r.state(mid).gates)==n; behavior['ordinary_autonomous_confirm']=a is not None and a.outcome=='CONFIRMED_DEFECT'
    if a:
     ...
     try:
      h={**b,'candidate_id':s['candidate_id'],'defect_assessment_ref':ar,'defect_assessment_digest':a.defect_assessment_digest,'quality_version_ref':qv,'campaign_refs':camps}; _,first_handoff_exc=invoke(lambda:worker(orch,'handoff_confirmed_defect',h)); c1=len(R43ApplicationService(rt).state(mid).confirmed_defect_lifecycles) if first_handoff_exc is None else 0; _,second_handoff_exc=invoke(lambda:worker(orch,'handoff_confirmed_defect',h)) if first_handoff_exc is None else (None,first_handoff_exc); c2=len(R43ApplicationService(rt).state(mid).confirmed_defect_lifecycles) if second_handoff_exc is None else c1; behavior['r43_real_service_called']=first_handoff_exc is None and bool(calls) and c1==1; behavior['r43_exact_handoff_idempotent']=second_handoff_exc is None and c1==c2==1
      lifecycles=R43ApplicationService(rt).state(mid).confirmed_defect_lifecycles
