  ...
  with tempfile.TemporaryDirectory(prefix='g5-human-') as td:
   ...
   try:
    ...
    if g:
     ...
     _,successor_exc=invoke(lambda:command('DEFECT_HUNTER','assess_defect_truth',ap(nb,s,HIGH))); behavior['successor_binding_confirms']=successor_exc is None and count(rt,mid)==before+1
   finally:
  ...
  with tempfile.TemporaryDirectory(prefix='g5-ordinary-') as td:
   ...
   try:
    ...
    if a:
     ...
     try:
      h={**b,'candidate_id':s['candidate_id'],'defect_assessment_ref':ar,'defect_assessment_digest':a.defect_assessment_digest,'quality_version_ref':qv,'campaign_refs':camps}; _,first_handoff_exc=invoke(lambda:command('DEFECT_HUNTER','handoff_confirmed_defect',h)); c1=len(R43ApplicationService(rt).state(mid).confirmed_defect_lifecycles) if first_handoff_exc is None else 0; _,second_handoff_exc=invoke(lambda:command('DEFECT_HUNTER','handoff_confirmed_defect',h)) if first_handoff_exc is None else (None,first_handoff_exc); c2=len(R43ApplicationService(rt).state(mid).confirmed_defect_lifecycles) if second_handoff_exc is None else c1; behavior['r43_real_service_called']=first_handoff_exc is None and bool(calls) and c1==1; behavior['r43_exact_handoff_idempotent']=second_handoff_exc is None and c1==c2==1
